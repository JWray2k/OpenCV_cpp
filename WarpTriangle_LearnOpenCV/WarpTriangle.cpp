...
// This example doesn't use Homography, but it gives and idea of how images automatically scale when resizing the image box.
// Triangle Warping using OpenCV - https://www.learnopencv.com/warp-one-triangle-to-another-using-opencv-c-python/ 
...
void warpTriangle(Mat &img1, Mat &img2, vector<Point2f> tri1, vector<Point2f> tri2)
{
	...
	warpAffine(img1Cropped, img2Cropped, warpMat, img2Cropped.size(), INTER_LINEAR, BORDER_REFLECT_101); //key algorithm here

	// Get mask by filling triangle
	Mat mask = Mat::zeros(r2.height, r2.width, CV_32FC3);
	fillConvexPoly(mask, tri2CroppedInt, Scalar(1.0, 1.0, 1.0), 16, 0);
	// Finally, we create a triangular mask by filling pixels inside the output triangle with white. This mask when multiplied 
	// with the output image turns all pixels outside the triangle black while preserving the color of all pixels inside the triangle.
...
int main(int argc, char** argv)
{
	...
	Mat imgIn = imread("C:/Users/wray/Pictures/robot.jpg");
