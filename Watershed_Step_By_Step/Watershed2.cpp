...
int main(int, char** argv) //I can't even get through step 1 for displaying this image... That's what I'll focus on... It was because I didn't set up the wait key.
{
	...
	// height and width of an image - https://stackoverflow.com/questions/32971241/how-to-get-image-width-and-height-in-opencv
	...
	// cv::Mat img(src.cols, src.rows, CV_8UC3);

	// use fast 4-byte alignment (default anyway) if possible
	// glut::glPixelStorei(GL_PACK_ALIGNMENT, (img.step & 3) ? 1 : 4); // this was included in either #include <GL/glu.h> or #include <GL/glut.h>

	//for (int i = 0; i < markers.rows; i+2)
	//{
	//	for (int j = 0; j < markers.cols; j++)
	//	{
	//		if (dst_hnh.at<Vec3b>(i, j) == colors[1])
	//		{
	//			dst_hnh.at<Vec3b>(i, j) = colors[6]; //change the color... then it should come out striped hopefully...
	//		}
