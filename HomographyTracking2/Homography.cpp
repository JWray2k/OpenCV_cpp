...
Require: feature ray R˜i from L˜ and Ri from L (i ∈ N)
1:	Minimum error Errmin = ∞.
2:	Best homography matrix Mmin = identity matrix.
3:	Iteration it = 0.
4:	Assign k, t and d empirically.
5:	while it < k do
6:		Initialize C with randomly selected m pairs of feature rays from L and L˜ with
		color difference smaller than t.
7:		Estimate homography matrix M via SVD from C.
8:		for every point R˜j in L˜ not selected in C do
9:			Warp R˜j onto L with M.
10:			if it corresponds with a ray Rj in L with color difference smaller than t then
11:					Add R˜j , Rj to C.
12:			end if
13:		end for
14:		if the number of pairs in C is larger than d then
15:			Recompute the homography matrix M via SVD from C.
16:			Measure the current error E by warping each ray R˜i onto Ri and measure the
			spatial distance.
17:			if E < Errmin then
18:				Errmin = E.
19:				Mmin = M.
...
pg 78 - https://www.eecis.udel.edu/~zyu/MyProjects/150dpi.pdf
*/