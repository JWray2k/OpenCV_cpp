...
https://github.com/PacktPublishing/OpenCV3-Computer-Vision-Application-Programming-Cookbook-Third-Edition/blob/master/Chapter09/matcher.cpp
\*------------------------------------------------------------------------------------------*/
...
//Crashing - OpenCV Error: Bad arguement <image is empty or has incorrect depth  <!=CV_8U>> in cv::xfeatures2d::SIFT_Impl::detectAndCompute, file modules\xfeatures2d\src\sift.cpp, line 1044
...
int main()
{
	...
	// 1. Read input images
	cv::Mat image1 = cv::imread("C:/Users/wray/Pictures/church01.jpg", cv::IMREAD_GRAYSCALE); 
	cv::Mat image2 = cv::imread("C:/Users/wray/Pictures/church02.jpg", cv::IMREAD_GRAYSCALE);

	// python.... functions for other solutions... img1 = cv2.imread("C:/Users/wray/Pictures/church01.jpg", 0
	...
	// radius match
	float maxDist = 0.3; // Matcher.cpp(135): warning C4305: 'initializing' : truncation from 'double' to 'float'
	matches2.clear();
	matcher.radiusMatch(descriptors1, descriptors2, matches2,
		maxDist); // maximum acceptable distance // Matcher.cpp(138): warning C4244: 'argument' : conversion from 'double' to 'float', possible loss of data
	...
	int nmatches = 0;
	for (int i = 0; i< matches2.size(); i++) nmatches += matches2[i].size(); //Matcher.cpp(151): warning C4267: '+=' : conversion from 'size_t' to 'int', possible loss of data
